        ...
        while True:
            try:
                with open("csv/%d" % self.fileNumber) as f:
                    ...
                    for row in reader:
                        for nodeName in self.nodeNames:
                            self.data[self.fileNumber - 1][nodeName].append(
                                float(row[nodeName])
                            )
    ...
    def updateGraph(self):
        ...
        for nodeName in self.nodeNames:
            ...
            if self.showControls[nodeName].isChecked():
                self.graphs[nodeName].setData(
                    self.data[self.fileNumber]["time"],
                    self.data[self.fileNumber][nodeName],
                )
            else:
    ...
    def addCheckboxes(self):
        ...
        for num, nodeName in enumerate(self.nodeNames):
            ...
            box.setStyleSheet(
                ...
                % (COLOURS[num][0], COLOURS[num][1], COLOURS[num][2])
            )
